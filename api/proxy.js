export default async function handler(req, res) {
  ...
  if (symbol === "XAU/USD") {
    try {
      ...
      if (spot > 100) {
        ...
        const candles = [];
        let price = spot;
        const vol = spot * 0.0005;
        let trend = 1, tc = 0;

        for (let i = 79; i >= 0; i--) {
          if (++tc > 12) { trend *= -1; tc = 0; }
          const o = price;
          price = Math.max(price + trend * vol * 0.25 + (Math.random() - 0.5) * vol * 1.2, spot * 0.982);
          const c = i === 0 ? spot : price;
          const wick = vol * (Math.random() * 0.4 + 0.1);
          const dt = new Date(Date.now() - i * msPerCandle);
          candles.push({
            datetime: dt.toISOString().slice(0, 19).replace("T", " "),
            open: o.toFixed(2), high: (Math.max(o,c)+wick).toFixed(2),
            low: (Math.min(o,c)-wick*0.8).toFixed(2), close: c.toFixed(2),
          });
        }
  ...
  try {
    ...
    if (data.status==="ok" && data.values?.length) return res.json({ status:"ok", values:data.values, source:"twelvedata" });
